...
function setSrcImagen(idImg,srcStr){
  if(srcStr instanceof FileBinario){
    srcStr=URL.createObjectURL(srcStr.datosBinarios);
  }else if(srcStr instanceof Blob){
    srcStr=URL.createObjectURL(srcStr);
  }
  //setSrcImagen(mng_img_Isquierda.idImagen_Imagen, URL.createObjectURL(mng_img_Isquierda.fileBinario.datosBinarios));
  let image = $('#'+idImg);
    ...
    aplicarConfiguracion(){
        $(this.idformulario).submit(function() {
            if (window.File && window.FileReader && window.FileList && window.Blob) {
              let file = $('#'+this.idInputFile)[0].files[0];
...
class Datos_uploadAjaxImagen_lista_todasLasImagenesObligado{
  constructor(){
    this.keyByts="file";
    this.keyNombreImagen="filename";
    this.keySize="size";
    this.keyEnvioImagen="envioImagen";
    this.keyCantidadDeImagenes="keyCantidadDeImagenes";
  }
}

function uploadAjaxImagen_lista_todasLasImagenesObligado(
  lista_idFile,urlPost,dicExtra={}
  ,evento_siTieneExito,evento_SiDaError
  ,d
  ){

    const keyByts=d.keyByts;
    const keyNombreImagen=d.keyNombreImagen;
    const keySize=d.keySize;
    const keyEnvioImagen=d.keyEnvioImagen;
    const keyCantidadDeImagenes=d.keyCantidadDeImagenes;
      console.log('llamo a enviar');

      function getStrKey(keyActual,indice){
        let r=keyActual+"_"+indice;
        return r;
      }
      let cantidadDeImg=lista_idFile.length;
      const formData = new FormData();
      for (let i = 0; i < lista_idFile.length; i++) {
        const idFile = lista_idFile[i];
        let hayImagen=notEmpty_ArchivoEnInput(idFile);
        dicExtra[getStrKey(keyEnvioImagen,i)]=hayImagen;
        ...
        if(hayImagen){
          ...
          formData.append(getStrKey(keyByts,i), currentChunk);
            formData.append(getStrKey(keyNombreImagen,i), file.name);
            formData.append(getStrKey(keySize,i), file.size);
      ...
      dicExtra[keyCantidadDeImagenes]=cantidadDeImg;
      for (const inputKey in dicExtra) {
              // console.log("inputKey"+inputKey);
              // console.log("dicExtra[inputKey]"+dicExtra[inputKey]);
              formData.append(inputKey, dicExtra[inputKey]);
